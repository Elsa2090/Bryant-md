...
zokou({
  ...
  if (!arg[0]) {
    repondre("wrong!!! Ie. .Play hozambe by Bryant ft Beltah._");
    return;
  }
...
_*BRYANT-MD IS DOWNLOADING YOUR FILE📂*_`
       }
...
zokou({
  ...
  if (!arg[0]) {
    repondre("insert video name Ie. _video hozambee by Bryant ft Beltah._");
    return;
  }
  ...
  try {
    ...
    if (videos && videos.length > 0 && videos[0]) {
      ...
      let InfoMess = {
        image: { url: videos[0].thumbnail },
        caption: `*⏣BRYANT-MD VIDEO DOWNLOADER⏣*\n\n*Video name :* _${Element.title}_
...
\n\n_*BRYANT MD IS DOWNLOADING YOUR FILE📂*_`
      ...
      fileStream.on('finish', () => {
        ...
        zk.sendMessage(origineMessage, { video: { url :"./video.mp4"} , caption: "*⏣GENERATED BY BRYANT MD⏣*", gifPlayback: false }, { quoted: ms });
      });
